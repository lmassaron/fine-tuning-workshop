...
class TestEnvironmentSanity(unittest.TestCase):
    def test_imports_and_torch(self):
        ...
        self.assertTrue(torch.cuda.is_available(), "CUDA is not available on this device!")
    ...
    def test_dataset_mapping(self):
        ...
        def format_data(examples):
            return {"text": [f"<|im_start|>user\n{q}\n<|im_end|>\n<|im_start|>assistant\n{r}<|im_end|>" for q, r in zip(examples["question"], examples["response"])]}
    ...
    def test_sft_config(self):
        ...
        config = SFTConfig(dataset_text_field="text", max_length=2048, eos_token="<|im_end|>")
