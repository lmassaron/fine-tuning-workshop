...
class LoRAManager:
    ...
    def __init__(self):
        ...
        print(f"[LoRAManager] Base model loaded. Memory footprint is stable.\n")
    ...
    def set_role(self, role_name: str):
        ...
        if role_name != "base":
            try:
                self.model.load_adapter(LORA_ADAPTERS[role_name], adapter_name=role_name)
            ...
            try:
                ...
                pass
            print(f"[LoRAManager] ⚡ Swapped -> Active Role: BASE")
...
def run_agent(user_request: str):
    ...
    if not steps:
        print("⚠️ Warning: No numbered steps found in plan. Attempting fallback split...")
    ...
    for step in steps:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if tool_name == "write_file":
                    result = write_file(tool_data.get("path", ""), tool_data.get("content", ""))
        ...
        if 'FAIL' in review.upper():
            print(f"  🚨 Reviewer flagged the step as FAILED.")
        else:
            print(f"  👍 Reviewer passed the step.")
            
    print("\n✨ Mission Complete.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Coding Multi-Agent CLI")
    parser.add_argument("instruction", type=str, nargs="?", default="", help="The instruction/task for the agent to perform.")
